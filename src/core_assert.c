...
#define _get_var_name(var) #var

static inline void _print_bool(bool var, const char *var_name) {
	printf("bool %s = %s", var_name, (var ? "true" : "false"));
}

static inline void _print_size_t(size_t var, const char *var_name) {
	printf("size_t %s = %ld", var_name, var);
}

static inline void _print_size_t_ptr(size_t *var, const char *var_name) {
	printf("size_t *%s = %p", var_name, var);
}

static inline void _print_char(char var, const char *var_name) {
	printf("char %s = %c", var_name, var);
}

static inline void _print_int(int var, const char *var_name) {
	printf("int %s = %d", var_name, var);
}

static inline void _print_long(long var, const char *var_name) {
	printf("long %s = %ld", var_name, var);
}

static inline void _print_float(float var, const char *var_name) {
	printf("float %s = %f", var_name, var);
}

static inline void _print_double(double var, const char *var_name) {
	printf("double %s = %lf", var_name, var);
}

static inline void _print_uchar(unsigned char var, const char *var_name) {
	printf("unsinged char %s = %u", var_name, var);
}

static inline void _print_uint(unsigned int var, const char *var_name) {
	printf("unsigned int %s = %u", var_name, var);
}

static inline void _print_ulong(unsigned long var, const char *var_name) {
	printf("unsigned long %s = %lu", var_name, var);
}

static inline void _print_char_ptr(char *var, const char *var_name) {
	printf("char *%s = %s", var_name, var);
}

static inline void _print_int_ptr(int *var, const char *var_name) {
	printf("int *%s = %p", var_name, var);
}

static inline void _print_long_ptr(long *var, const char *var_name) {
	printf("long *%s = %p", var_name, var);
}

static inline void _print_float_ptr(float *var, const char *var_name) {
	printf("float *%s = %p", var_name, var);
}

static inline void _print_double_ptr(double *var, const char *var_name) {
	printf("double *%s = %p", var_name, var);
}

static inline void _print_uchar_ptr(unsigned char *var, const char *var_name) {
	printf("unsigned char *%s = %p", var_name, var);
}

static inline void _print_uint_ptr(unsigned int *var, const char *var_name) {
	printf("unsigned int *%s = %p", var_name, var);
}

static inline void _print_ulong_ptr(unsigned long *var, const char *var_name) {
	printf("unsigned long *%s = %p", var_name, var);
}

static inline void _print_map_t_ptr(struct Map *var, const char *var_name) {
	printf("struct Map *%s = %p", var_name, var);
}

static inline void _print_map_t(struct Map var, const char *var_name) {
	printf("struct Map %s = {\n\t.rows = %ld,\n\t.cols = %ld,\n\t.drake_wake_time = %ld\n}",
					var_name, var.rows, var.cols, var.drake_wake_time);
}

static inline void _print_default(void *var, const char *var_name) {
...
void map_allocation_test(size_t rows, size_t cols, size_t drake_wake_time, bool verbose)
{
	...
	for (size_t i = 0; i < rows; i++) {
		...
		cells[i] = (char *) malloc(sizeof(char) * cols);
